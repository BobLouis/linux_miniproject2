...
int main(int argc, char const *argv[])
{
	...
	while (1)
	{
		...
		memset(num1, '\0', sizeof(num1) / sizeof(char));
		memset(num2, '\0', sizeof(num2) / sizeof(char));
		...
		if (!strcmp(str, "add")) //add function
		{
			...
			{										   // long long range -9,223,372,036,854,775,808 to 9,223,372,036,854,775,807
				if (strlen(num1) >= 21)				   //if string len > 21 exceed the max of long long
					overflow = 1;
				else if (strlen(num1) == 20 && (*num1 != '-' || a > 0)) //if the strlen == 20 check if the number is negative
					overflow = 1;
				else if (strlen(num1) == 19 && *num1 != '-' && a < 0) //if the strlen == 19 check if the number overflow if overflow the number will resent negative
					overflow = 1;
			}
			if (strlen(num2) >= 19)
			{
				if (strlen(num2) >= 21)
					...
					overflow = 1;
				else if (strlen(num2) == 19 && *num2 != '-' && b < 0)
					overflow = 1;
			}
			if (overflow)
